...
ensure_api_tls_cert() {
  local cert="${API_TLS_DIR}/api.crt"
  local key="${API_TLS_DIR}/api.key"
  mkdir -p "${API_TLS_DIR}"
  if [[ -f "${cert}" && -f "${key}" ]]; then
    return
  fi
  log "Generating self-signed TLS certificate for IP API endpoint"
...
render_api_ip_vhost() {
  ...
  sed \
    -e "s|__API_CERT__|${cert}|g" \
    -e "s|__API_KEY__|${key}|g" \
  ...
  ensure_api_nginx_permissions
  log "Enabled IP API vhost ( /api/game/url-extended/<client> )"
}
