...
ensure_runtime_dirs() {
  mkdir -p "${PROXIES_ETC}" "${PROXIES_STATE}" "${DOMAINS_DIR}" "${API_TLS_DIR}" "${CLIENTS_DIR}" "${URLS_DIR}"
  ...
  chmod 755 "${API_TLS_DIR}" 2>/dev/null || true
}

# Make Basic Auth + API TLS files readable by the nginx worker (www-data).
ensure_api_nginx_permissions() {
  ...
  if [[ -f "${API_TLS_DIR}/api.crt" ]]; then
    chmod 644 "${API_TLS_DIR}/api.crt"
  fi
  if [[ -f "${API_TLS_DIR}/api.key" ]]; then
    # Master process (root) loads the key; keep it root-only.
    chmod 600 "${API_TLS_DIR}/api.key"
    chown root:root "${API_TLS_DIR}/api.key" 2>/dev/null || true
  fi
}
