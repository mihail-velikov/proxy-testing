...
`--client` is **repeatable** (legacy `--client-name` still works for a single client). Each client gets `/etc/proxies/clients/<name>.env`.
...
After at least one successful rotation, the VM exposes an HTTPS endpoint on its **public IP** (self-signed certificate). Use Basic Auth credentials from install (`--api-user` / `--api-password`) — the **same** password for every client. Append the client id to the path:

```bash
VM_IP="$(curl -4 -fsS https://ifconfig.me)"

curl -sk --location "https://${VM_IP}/api/game/url-extended/clientname42" \
...
```bash
curl -sk -u 'domainapi:CHOOSE_A_STRONG_PASSWORD' "https://${VM_IP}/api/game/url-extended/clientname42"
```
...
```json
{
  ...
  "status": "OK",
  "urls": {
    "clientname42-gc-prod-bgsp": "https://clientname42-gc-prod-bgsp.NEWdomain.com",
    "lottery-api-instant": "https://lottery-api-instant.NEWdomain.com"
  }
}
```
...
- `-sk` is needed because the IP API uses a **self-signed** TLS certificate.
...
- Origin hostnames come from `/etc/proxies/prefixes-client.env` (`__CLIENT__` → each client) plus `/etc/proxies/prefixes-shared.env`.
...
|---------|----------------|
| Install fails downloading files | Confirm the GitHub repo is **public**, `--base-url` is correct, and `--github-ref` exists |
| Rotation stops at registrant | Edit `/etc/proxies/registrant.env` with real contact data |
| Domain/Create fails | InternetBS balance, API key/password, registrant fields |
| Certificate fails | DNS A records, InternetBS nameservers, wait for propagation; check hooks under `/opt/proxies/hooks/` |
| Site not reachable | `sudo nginx -t`, `systemctl status nginx`, DNS pointing to this VM’s public IP |
| nginx exits: `host not found in upstream` | `CDN_ORIGIN` / `BACKEND_ORIGIN` must be a real hostname (FQDN) or IP in `/etc/proxies/credentials.env` — not a placeholder like `cdn_backend`. Fix the value, re-render sites (`sudo /opt/proxies/scripts/force-rotate.sh` or resume), then `sudo nginx -t && sudo systemctl reload nginx`. Newer templates resolve upstream DNS per-request so a temporary DNS failure no longer prevents nginx from starting. |
| `could not build server_names_hash` / increase `server_names_hash_bucket_size` | Long multi-client `server_name` lists need a larger hash. Install drops `/etc/nginx/conf.d/proxies-server-names-hash.conf`. On an existing VM: `echo -e 'server_names_hash_bucket_size 128;\nserver_names_hash_max_size 2048;' \| sudo tee /etc/nginx/conf.d/proxies-server-names-hash.conf` then resume: `sudo /opt/proxies/scripts/force-rotate.sh --resume-domain YOURDOMAIN.com` |
| `api.htpasswd` Permission denied in nginx error log | `/etc/proxies` must be traversable and `api.htpasswd` readable by `www-data`. Fix (or re-run install): `sudo chmod 755 /etc/proxies /var/lib/proxies && sudo chown root:www-data /etc/proxies/api.htpasswd && sudo chmod 640 /etc/proxies/api.htpasswd && sudo nginx -s reload` |
| Cron not running | `ls /etc/cron.d/proxies-domain-rotation`, check `/var/log/proxies-rotate.log` |

Detect public IP the script uses:
...
## Support checklist to send your operator

If something fails, gather:

1. Output of `sudo /opt/proxies/scripts/force-rotate.sh` (or `/var/log/proxies-rotate.log`)
2. `sudo cat /var/lib/proxies/current-domain`
3. `sudo ls /var/lib/proxies/domains/`
4. `sudo nginx -t`
5. Ubuntu version: `lsb_release -a` (must be **26.04**)
