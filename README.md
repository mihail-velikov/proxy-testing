...
```bash
curl -sk -u 'domainapi:STRONG_PASSWORD' "https://VM_PUBLIC_IP/api/game/url-extended/clientname42"
```
...
## Dry-run install (local checkout)

```bash
sudo ./install.sh \
  --api-key test --password test \
  --client alpha --client beta \
  --cdn-origin cdn.example-upstream.test \
  --backend-origin backend.example-upstream.test \
  --email ops@example.com \
  --api-user apiuser --api-password apipass \
  --local-dir . \
  --prefix /tmp/proxies-test \
  --skip-packages --skip-cron --allow-non-root
```
...
- URL API: `GET /api/game/url-extended/<client>` with shared Basic Auth (`--api-user` / `--api-password`).
